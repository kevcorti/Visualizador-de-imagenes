 ...
 */
public class PantallaDetallesEliminarPersonasController implements Initializable {
    ...
    private void eliminarPersona(MouseEvent event) {
        try{
            ...
            if(!(nombrePersonas.contains(infoPersona.getText().toUpperCase())))
                throw new PersonaExistenteException("Esa persona no se encuentra etiquetada en esta imagen");
            nuevasPersonas = new ArrayList<Persona>(1);
            for(String s: nombrePersonas){
                if(!(Objects.equals(s,infoPersona.getText().toUpperCase())))
                    nuevasPersonas.addLast(new Persona(s));
            }
            if(nuevasPersonas.size() > 0)
                imagen.setPersonas(nuevasPersonas);
            else{
                imagen.setPersonas(null);
            }
            ...
            sg.show();
            Alert a = new Alert(AlertType.CONFIRMATION, "Persona eliminada");
            a.show();
        }
