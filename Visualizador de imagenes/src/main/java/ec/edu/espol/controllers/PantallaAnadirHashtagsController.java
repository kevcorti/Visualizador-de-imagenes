 ...
 */
public class PantallaAnadirHashtagsController implements Initializable {
    ...
    private void enviarInfo(MouseEvent event) {
        try{
            ...
            pafc.recibirDatosHashtags(album, lugar, dia, mes, an, desc, modCam, marCam, personas, hashtags, file);
            Stage sg = new Stage();
            sg.setScene(sc);
    ...
    private void regresarAnterior(MouseEvent event) {
        try{
            ...
            PantallaAgregarFotoController pafc = loader.getController();
            pafc.recibirUsuario(this.usuario);
            pafc.regresarDesdeHashtags(album, lugar, dia, mes, an, desc, modCam, marCam, personas, null, file);
            Stage sg = new Stage();
            sg.setScene(sc);
            sg.setTitle("¡Organiza tus fotos!");
            String rut = System.getProperty("user.dir") + "/src/main/resources/img/icono.png";
    ...
    private void verificarCantidad(MouseEvent event) {
        try{
            ...
            if(cantidad <= 0 ){
                ...
                if(cantidad == 1)
                    lblPersonas.setText("Hashtag");
                else{
                    cantidad2 = 1;
                    lblPersonas.setText("Hashtag #1");
                }
            }
        }
    ...
    private void anadirPersonas(MouseEvent event) {
        try{
            ...
            if(cantidad == 1){
                hashtags.addLast(infoPersonas.getText());
                infoPersonas.setDisable(true);
                btnAnadir.setDisable(true);
                ...
                cantidad2++;
                cantidad--;
                lblPersonas.setText("Hashtag #" + cantidad2);
            }
        }
        catch(PanelVacioException ex){
            Alert a = new Alert(Alert.AlertType.ERROR, ex.getMessage());
    ...
    public void recibirDatos(String album, String lugar, String dia, String mes, String an, String desc, String modCam, String marCam, ArrayList<Persona> personas, File archivoImagen){
        this.album = album;
        this.lugar = lugar;
        this.dia = dia;
        this.mes = mes;
