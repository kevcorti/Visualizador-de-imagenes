 ...
 */
public class PantallaBuscarAlbumPersonaYLugarController implements Initializable {
    ...
    @FXML
    private TextField infoCantidadPersonas;
    @FXML
    private TextField infoPersonas;
    @FXML
    private Button btnCantidadPersonas;
    @FXML
    ...
    private void buscarAlbumes(MouseEvent event) {
        try{
            if(Objects.equals(infoLugar.getText(),""))
                throw new PanelVacioException("Ingrese el nombre del lugar por favor");
                ...
                if(!(Objects.equals(imagen.getPersonas(),null))){
                    if(imagen.getPersonas().size() >= personas.size()){
                        ...
                        for(Persona persona: personas){
                            if(nombrePersona.contains(persona.getNombre()) && Objects.equals(infoLugar.getText().toUpperCase(),imagen.getLugar())){
            ...
            if(albumesResultados.size() == 0)
                throw new PersonasExistentesException("No hay fotos en las que aparezcan todas las personas en ese lugar");
    ...
    private void verificarCantidad(MouseEvent event) {
        try{
            if(Objects.equals(infoCantidadPersonas.getText(),""))
                throw new PanelVacioException("Ingrese el valor por favor");
            int cantidad = Integer.parseInt(infoCantidadPersonas.getText());
            if(cantidad <= 0 ){
                ...
                infoCantidadPersonas.setDisable(true);
                btnCantidadPersonas.setDisable(true);
    ...
    private void anadirPersona(MouseEvent event) {
        try{
            ...
            if(cantidad == 1){
                ...
                cantidad2++;
                cantidad--;
                lblPersona.setText("Nombre #" + cantidad2);
            }
        }
        catch(PanelVacioException ex){
            Alert a = new Alert(Alert.AlertType.ERROR, ex.getMessage());
    ...
    private void reiniciarDatos(MouseEvent event) {
        this.personas = null;
        infoCantidadPersonas.setDisable(false);
        btnCantidadPersonas.setDisable(false);
        lblPersona.setText("");
        infoPersonas.setText("");
        infoPersonas.setDisable(true);
        btnAnadir.setDisable(true);
